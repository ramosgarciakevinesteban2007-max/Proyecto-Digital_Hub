// frontend_react/src/components/ChatFicha.jsx
...
const ChatFicha = ({ idFicha, token }) => {
  ...
  useEffect(() => {
    cargar();
    const interval = setInterval(cargar, 5000); // polling cada 5s
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'420px',background:'#0f0820',border:'1px solid rgba(127,90,240,0.3)',borderRadius:'16px',overflow:'hidden'}}>
      <div style={{flex:1,overflowY:'auto',padding:'16px',display:'flex',flexDirection:'column',gap:'10px'}}>
        {loading ? <div style={{color:'#b8a8d8',fontSize:'13px'}}>Cargando...</div>
        : mensajes.length === 0 ? <div style={{color:'#b8a8d8',fontSize:'13px',textAlign:'center',marginTop:'40px'}}>No hay mensajes aún. ¡Sé el primero!</div>
        : mensajes.map(m => {
          ...
          return (
            <div key={m.id} style={{display:'flex',flexDirection:'column',alignItems: esMio ? 'flex-end' : 'flex-start'}}>
              ...
              <div style={{fontSize:'10px',color:'#6a5a8a',marginTop:'3px'}}>{new Date(m.fecha_envio).toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'})}</div>
            </div>
          );
        })}
        <div ref={bottomRef}/>
